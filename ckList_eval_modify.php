					...
					<td align="left">
						...
						<b>
							<?
								...
								//평가 DB에 내가 입력한 값 불러오기		
								$ck_eval_me_sql = "
										SELECT * FROM nurse2016_ck_eval where n_key = '$n_key' and p_key = '$p_key' and pbList_num = '$pbNum';
									   ";		
								$ck_eval_me_result = @mysql_query($ck_eval_me_sql,$db_info);	
					

								//내가 입력한 내용
								mysql_data_seek($ck_eval_me_result, 0);
								$ck_eval_me_db = mysql_fetch_array($ck_eval_me_result);



							?>
							<label>&nbsp;
								<input type="radio" style="width:23px;height:23px;vertical-align: middle;"  name="sur_<?=$pLNum?>" value= "5" <? if($ck_eval_me_db['EvalScore'] == "5") echo "checked"?>>&nbsp;5점
							</label>

							<label>&nbsp;
								<input type="radio" style="width:23px;height:23px;vertical-align: middle;"  name="sur_<?=$pLNum?>" value= "4" <? if($ck_eval_me_db['EvalScore'] == "4") echo "checked"?>>&nbsp;4점
							</label>

							<label>&nbsp;
								<input type="radio" style="width:23px;height:23px;vertical-align: middle;"  name="sur_<?=$pLNum?>" value= "3" <? if($ck_eval_me_db['EvalScore'] == "3") echo "checked"?>>&nbsp;3점
							</label>

							<label>&nbsp;
								<input type="radio" style="width:23px;height:23px;vertical-align: middle;"  name="sur_<?=$pLNum?>" value= "2" <? if($ck_eval_me_db['EvalScore'] == "2") echo "checked"?>>&nbsp;2점
							</label>

							<label>&nbsp;
								<input type="radio" style="width:23px;height:23px;vertical-align: middle;"  name="sur_<?=$pLNum?>" value= "1" <? if($ck_eval_me_db['EvalScore'] == "1") echo "checked"?>>&nbsp;1점
							</label>
						</b>
						</font>
					</td>
					...
					<td align="center">
						...
						<b>
							<?
								...
								//echo $ck_eval_me_db['EvalScore'];

								//세 명이 입력한 평가의 평균
								//echo "bbb=".$ck_eval1_db['EvalScore']." ";
								//echo "ccc=".$ck_eval2_db['EvalScore']."<br>";
								echo round(($ck_eval1_db['EvalScore'] + $ck_eval2_db['EvalScore'] + $ck_eval_me_db['EvalScore'])/3, 2);
							?>
						</b>
						</font>
					</td>
