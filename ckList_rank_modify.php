...
<?
		...
		//DB에 입력된 값 불러오기(현재 입력자)		
		$ckUser_sql = "
				SELECT ckRank_1, ckRank_2, ckRank_3, ckRank_4, ckRank_5, ckRank_6, ckRank_7, ckRank_8, ckRank_9, ckRank_10, ckRank_11, ckRank_12, ckRank_13, ckRank_14, ckRank_15, ckRank_16, ckRank_17, ckRank_18 FROM nurse2016_ck_rank where n_key = '$n_key' and p_key = '$p_key';
			   ";
		$ckUser_result = @mysql_query($ckUser_sql, $db_info);

		//DB에 입력된 값 불러오기(입력자1, 입력자2)		
		$ck_rank_sql = "
				...
				<tr>
					...
					<td width = "10%"  align="center">
						<select name="sur_rank_<?echo $pbList['pbList_num']?>" style="width:75px;height:30px"">
							...
							<?
								//현재 사용자가 입력한 값 보여주기
								mysql_data_seek($ckUser_result, 0);
								$ck_rank_db = mysql_fetch_array($ckUser_result);
							...
							?>
							<option value="<?echo $j?>" <? if($ck_rank_db[$ckRankNum] == $j) echo "selected = 'selected'"?>><?echo $j?>순위</option>
					...
					<td width = "10%"  align="center">
						<font><? $rank_avg = ($ck_rank_db[$ckRankNum] + $ck_rank1_db[$ckRankNum]+$ck_rank2_db[$ckRankNum])/3; echo round($rank_avg,2);?></font>
					</td>
				</tr>
